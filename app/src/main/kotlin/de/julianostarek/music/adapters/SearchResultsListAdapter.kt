...
class SearchResultsListAdapter(private val results: List<MediaObject>) : RecyclerView.Adapter<UniversalViewHolder<*>>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UniversalViewHolder<*> {
        when (viewType) {
            1 -> {
                val ui = MediaItemViewHolders.Big.SingleLine.UI()
                return MediaItemViewHolders.Big.SingleLine(ui.createView(AnkoContext.create(parent.context, parent)), ui, parent.context as AppCompatActivity)
            }
            2 -> {
                val ui = MediaItemViewHolders.Big.UI()
                return MediaItemViewHolders.Big<Album>(ui.createView(AnkoContext.create(parent.context, parent)), ui, parent.context as AppCompatActivity)
            }
            3 -> {
                val ui = MediaItemViewHolders.Small.Search.UI.SingleLine()
                return MediaItemViewHolders.Small.Search(ui.createView(AnkoContext.create(parent.context, parent)), ui, parent.context as AppCompatActivity)
            }
            4 -> {
                val ui = MediaItemViewHolders.Small.Search.UI()
                return MediaItemViewHolders.Small.Search(ui.createView(AnkoContext.create(parent.context, parent)), ui, parent.context as AppCompatActivity)
            }
    ...
    override fun getItemViewType(position: Int): Int {
        when (position) {
            0 -> return if (results.first() is Artist) 1 else 2
            else -> return if (results.first() is Artist) 3 else 4
        }
    }
